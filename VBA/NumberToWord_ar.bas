...
End Function
 ...
 Function NumberToWordAR(num As Double) As String
	...
	zeroToNineteen = Array("ÕÝÑ", "æÇÍÏ", "ÅËäÇä", "ËáÇËÉ", "ÃÑÈÚÉ", "ÎãÓÉ", "ÓÊÉ", "ÓÈÚÉ", "ËãÇäíÉ", "ÊÓÚÉ", "ÚÔÑÉ", "ÅÍÏì ÚÔÑ", "ÅËäÇ ÚÔÑ", "ËáÇËÉ ÚÔÑ", "ÃÑÈÚÉ ÚÔÑ", "ÎãÓÉ ÚÔÑ", "ÓÊÉ ÚÔÑ", "ÓÈÚÉ ÚÔÑ", "ËãÇäíÉ ÚÔÑ", "ÊÓÚÉ ÚÔÑ")
	
	Dim multiplesOf10 As Variant
	multiplesOf10 = Array("", "ÚÔÑÉ", "ÚÔÑæä", "ËáÇËæä", "ÃÑÈÚæä", "ÎãÓæä", "ÓÊæä", "ÓÈÚæä", "ËãÇäæä", "ÊÓÚæä")
	
	Dim multiplesOf100 As Variant
	multiplesOf100 = Array("", "ãÇÆÉ", "ãÇÆÊÇä")
	
	Dim multiplesOf1000Singular As Variant
	multiplesOf1000Singular = Array("", "ÃáÝ", "ãáíæä", "Èáíæä", "ÊÑíáíæä", "ßæÇÏÑíáíæä", "ßæíäÊíáíæä", "ÓíßÓÊíáíæä", "ÓíÈÊíáíæä", "ÃæßÊíáíæä", "äæäíáíæä")
	
	Dim multiplesOf1000Dual As Variant
	multiplesOf1000Dual = Array("", "ÃáÝÇä", "ãáíæäÇä", "ÈáíæäÇä", "ÊÑíáíæäÇä", "ßæÇÏÑíáíæäÇä", "ßæíäÊíáíæäÇä", "ÓíßÓÊíáíæäÇä", "ÓíÈÊíáíæäÇä", "ÃæßÊíáíæäÇä", "äæäíáíæä")
	
	Dim multiplesOf1000Plural As Variant
	multiplesOf1000Plural = Array("", "ÃáÇÝ", "ãáÇííä", "ÈáÇííä", "ÊÑíáíæäÇÊ", "ßæÇÏÑíáíæäÇÊ", "ßæíäÊíáíæäÇÊ", "ÓíßÓÊíáíæäÇÊ", "ÓíÈÊíáíæäÇÊ", "ÃæßÊíáíæäÇÊ", "äæäíáíæÊ")
	...
	For i = 0 To segmentsCount
		...
		If p1 > 0 Then
			...
			Else
				' Remove the ta'a when related to 100, it must be for example looks like ËáÇË ãÇÆÉ
				word.Add Replace(zeroToNineteen(p1), "É", "") & " " & multiplesOf100(1)
			End If
		End If
		...
		Else
			If segmentIndex > 0 Then
				...
				Else
					' segmentVal >= 11
					segmentMultiplier = " " & multiplesOf1000Singular(segmentIndex) & "Çð"
				End If
			End If
		End If
		...
		wordStr = Join(CollectionToArray(word), " æ ")
...
NextLoop:
    ...
    If wholeWords.count > 0 Then
	    NumberToWordAR = Join(CollectionToArray(wholeWords), " æ ")
    End If
End Function
